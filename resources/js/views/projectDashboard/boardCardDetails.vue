<template>
    <div class="container" v-if="Object.keys(selectedData).length > 0">
        <div class="row">
            <div class="col-3">
                <div style="margin-top: 5px">
                    <span v-if="selectedData.multiple_board_id && selectedData.progress" class="go-details" @click="showOriginalList(selectedData)">
                        Go to Board
                    </span>
                </div>
                <div>
                    <span v-if="selectedData.list_id && selectedData.type !== 'task'" class="go-details" @click="showOriginalList(selectedData)">
                        Go to List
                    </span>
                </div>
            </div>
                                            ...
                                            <div class="col-12 tag-section">
                                                ...
                                                <div v-if="searchTag">
                                                    <template
                                                        ...
                                                        <li @click="addExistingTagSearch(index , stagIndx, key, selectedData.cardId,searchAllTags, '')"
                                ...
                                <li class="assignUser auHeight">
                                    <template>
                                        <div v-for="user in allTeamsUsers" v-if="selectedData.assigned_user_ids.includes(user.id) === true"
                                            ...
                                            <div class="col-md-8 assign-user-name-email">
                                                <h5 style="padding-right: 17px;">{{user.name}}<br>
            ...
            <div>
                <span class="open-details" @click="showSingleView(selectedData)">
                    Open Details
                </span>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                <h5 > Created By <strong>: {{selectedData.created_by.name}}</strong></h5>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                <h5 class="detailsTitle" v-html="selectedData.data"></h5>
            </div>
        </div>
        <!-- Tabs -->
        <div class="row">
            <div class="col-12">
                <ul class="nav nav-tabs" id="myTab" role="tablist">
                    <li class="nav-item">
                        <a aria-controls="home" aria-selected="true" class="nav-link active" data-toggle="tab" href="#home" id="_details" role="tab">Details</a>
                    </li>
                    <li class="nav-item" @click="getFiles(selectedData.cardId)">
                        <a aria-controls="file" aria-selected="false" class="nav-link" data-toggle="tab" href="#file" id="_file" role="tab">Files</a>
                    </li>
                    <li class="nav-item" @click="getComments(selectedData.cardId)">
                        <a aria-controls="comment" aria-selected="false" class="nav-link" data-toggle="tab" href="#comment" id="_comment" role="tab">Comments</a>
                    </li>
                    <li class="nav-item">
                        <a @click="showLog" aria-controls="log" aria-selected="false" class="nav-link" data-toggle="tab" href="#log" id="_log" role="tab">Logs</a>
                    </li>
                    <li @click="showChild(selectedData.cardId)" class="nav-item" v-if="selectedData.type !== 'task' && (selectedData.childrens.length  > 0 || selectedData.parents.length  > 0)">
                        <a aria-controls="child" aria-selected="false" class="nav-link" data-toggle="tab" href="#child" id="_child" role="tab">Related Tasks</a>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="tab-content">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="textAreaExtend">
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="submitdetails" id="submitdetails" style="margin-top: 20px"></div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div aria-labelledby="comment-tab" class="tab-pane" id="comment" role="tabpanel" style="overflow: hidden;">
                <span>
                    <div class="row comment-section-in-task-details" style="max-height: calc(100vh - 300px); overflow-x: hidden;">
                                    ...
                                    <span style="padding: 10px;"
                                        ...
                                        <a target="_blank" :href="'/storage/'+selectedData.cardId+'/comment/'+reply.attatchment" style="cursor: pointer;">
                                            ...
                                            <div v-if="reply.attatchment.endsWith('.doc') || reply.attatchment.endsWith('.docx') || reply.attatchment.endsWith('.xls') || reply.attatchment.endsWith('.xlsx')">
                                                <img title="Click To Download" data-toggle="tooltip" :src="'/img/file.png'" height="50" width="50"></div>
                                        </a>
                                    </span>
                                </div>
                                <div class="comment_toolbar">
        ...
        <div class="row">
            <div class="col-1">
                <p :title="selectedData.userName" class="assignUser-photo-for-selected text-uppercase details-comments-pic"
                   data-placement="bottom" data-toggle="tooltip" style="overflow:hidden;"> {{
                    selectedData.userName.substring(0,2) }}</p>
            </div>
            <div class="col-11">
                <textarea @focus="ShowTextArea(selectedData)"
                          ...
                          @click="commentEdit($event, selectedData.cardId)"
            ...
            <div class="tags-log">
                <table class="table table-striped table-bordered">
                    <thead class="save-all">
                        <tr>
                            <th scope="col" style=" width: 34%;font-size: 13px">Title</th>
                            <th scope="col" style=" width: 33%;font-size: 13px">Log Type</th>
                            <th scope="col" style=" width: 16%;font-size: 13px">Action By</th>
                            <th scope="col" style=" width: 17%;font-size: 13px">Action At</th>
                        </tr>
                    </thead>
                    <tbody>
    ...
    export default {
        ...
        methods: {
            ...
            commentPress(evt, cardId, data) {
                ...
                if (_this.trigger === true && evt.which !== 16 && evt.which !== 50) {
                    setTimeout(() => {
                        ...
                        if (str.includes('@')) {
                            ...
                            if (notKeys.includes(evt.key) === false) {
                                _this.userNames += evt.key;
                            }
                ...
                if ((evt.shiftKey && evt.which === 50) || evt.key === '@') {
                    _this.trigger = true;
                    _this.commentsData = title; //$('.commentInput').val();
            ...
            commentReplyPress(reEvt, dataId, comments) {
                let _this = this;
                let title = comments;
                if (reEvt.which === 32 || reEvt.which === 13) {
                    _this.trigger = false;
                    _this.userNames = '';
                    _this.replyProjectUsers = null;
                    $('.myUL-user-comment').css({display: 'none'});
                }
                if (_this.trigger === true && reEvt.which !== 16 && reEvt.which !== 50) {
                    var lastIndex = title.lastIndexOf(" ");
                    let str = title.substring(lastIndex);
                    if (str.includes('@')) {
                        let notKeys = ["Backspace", "ScrollLock", "null", "NumLock", "Tab", "ArrowLeft", "ArrowDown", "ArrowRight", "ArrowUp"];
                        if (notKeys.includes(reEvt.key) === false) {
                            _this.userNames += reEvt.key;
                        }
                        if (reEvt.key === "Backspace") {
                            _this.userNames = _this.userNames.slice(0, -1);
                        }
                        axios.post('/api/task-list/search-result', {'user_name': _this.userNames})
                            .then(response => response.data)
                ...
                if (reEvt.shiftKey && reEvt.which === 50) {
                    _this.trigger = true;
                ...
                axios.post('/api/task-list/assign-user', postData)
                    ...
                    .then(response => {
                        if (response.success === 'success') {
                            setTimeout(function () {
                                _this.selectedData.assigned_user = response.data;
            ...
            addExistingTagSearch(index, tagIndx, key, cardId, allTags, dntfrgt = '') {
                let _this = this;
                if (dntfrgt !== '') {
                    var postData = {
                        id: cardId,
            ...
            showSingleView(task) {
                ...
                let routeData = this.$router.resolve({
                    ...
                    params: {projectId: nav_type.project_id, type: 'lists', task_id: btoa(task.cardId), id: task.list_id != null ? task.list_id : task.board_id  }
                });
