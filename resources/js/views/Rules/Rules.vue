    ...
    <div class="Rules">
        <div class="container rules-create">
            ...
            <section v-if="rules_action ===  'rules'">
                <div class="row rulesTableHeight">
                    <table class="table table-striped table-bordered">
                        <thead class="thead save-all">
                        <tr>
                            <th>#</th>
                            <th>Name</th>
                        ...
                        <tbody>
                        <template v-for="(rl, key) in all_rules">
                            <tr>
                                <td style="width: 3%;">{{key+1}}</td>
                                <td style="width: 23%;">{{rl.name}}</td>
                                <td style="width: 20%;">{{(rl.move_from !== null) ? rl.move_from.title : 0}}</td>
                                ...
                                <td style="width: 20%;">
                                    ...
                                    <span v-if="rl.assigned_users.length === 0"
                                          class="badge badge-warning" style="color: #fff">Not Assigned</span>
                                </td>
        ...
        methods: {
            ...
            AddNewRules() {
                ...
                if (_this.rule.move_from !== 0 && _this.rule.name !== '') {
                    ...
                    axios.post('/api/add-rules', _this.rule)
                        ...
                        .then(response => {
                            if (response.status === 'exist') {
                                ...
                                _this.Socket.emit('rulesCreate', {
                                    assign_users: _this.rule.assign_to,
                    ...
                    Swal.fire("Sorry!", 'Required Field Can\'t be empty', 'warning');
                }
            },
            UpdateRule() {
                var _this = this;
            ...
            DeleteRule(id) {
                var _this = this;
                Swal.fire(
                    Helper.swalDelete('Are you sure?', 'You want to delete this Rule !!!')
                ).then((result) => {
                    if (result.value) {
